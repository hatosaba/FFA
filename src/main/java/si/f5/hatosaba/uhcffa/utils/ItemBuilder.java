...
public final class ItemBuilder {
    ...
    private ItemBuilder(ItemStack item) {
        ...
        //Preconditions.checkArgument(item.getType().isBlock(), "Illegal material!");
    ...
    public ItemBuilder lore(String... lore) {
        if (lore == null || lore.length > 0) {
            this.meta.setLore(Arrays.stream(lore).map(s -> s.replaceAll("&", "§")).collect(Collectors.toList()));
        }
    ...
    public ItemBuilder lore(List<String> lore) {
        this.meta.setLore(lore.stream().map(s -> s.replaceAll("&", "§")).collect(Collectors.toList()));
        return this;
    }
