...
public enum ParticleEffect {
     ...
     * <li>Only the motion on the y-axis can be controlled, the motion on the x- and z-axis are multiplied by 0.1 when setting the values to
     * 0
     * </ul> */
    SPELL("spell", 13, -1),
    ...
    /** Displays a single particle which flies into a determined direction and is only visible for all players within a certain range in the
    ...
    /** Displays a particle effect which requires additional data and is only visible for all players within a certain range in the world of @param
     * center
     * @param data
     *            Data of the effect
     * @param offsetX
    ...
    /** Displays a single particle which requires additional data that flies into a determined direction and is only visible for all players
    ...
    /** Displays a single particle which requires additional data that flies into a determined direction and is only visible for the
     * specified players
     * @param data
     *            Data of the effect
     * @param direction
    ...
    /** Represents the particle data for effects like {@link ParticleEffect#ITEM_CRACK}, {@link ParticleEffect#BLOCK_CRACK} and
    ...
    /** Represents the color for effects like {@link ParticleEffect#SPELL_MOB}, {@link ParticleEffect#SPELL_MOB_AMBIENT} and
    ...
    /** Represents a runtime exception that is thrown either if the displayed particle effect requires data and has none or vice-versa or if
    ...
    /** Represents a runtime exception that is thrown either if the displayed particle effect is not colorable or if the particle color type
     * is incorrect
     * <p>
     * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
     * @author DarkBlade12
    ...
    public static final class ParticlePacket {
         ...
         * @see #initialize() */
        public ParticlePacket(ParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed, int amount,
        ...
        /** Initializes {@link #packetConstructor}, {@link #getHandle}, {@link #playerConnection} and {@link #sendPacket} and sets
         * {@link #initialized} to <code>true</code> if it succeeds
         * <p>
         * <b>Note:</b> These fields only have to be initialized once, so it will return if {@link #initialized} is already set to
         * <code>true</code>
        ...
        public static void initialize() throws VersionIncompatibleException {
            ...
            try {
                ...
                Class<?> packetClass = ReflectionUtil.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles"
                ...
                sendPacket = ReflectionUtil.getMethod(playerConnection.getType(), "sendPacket",
                        ReflectionUtil.PackageType.MINECRAFT_SERVER.getClass("Packet"));
            ...
            try {
                ...
                if (version < 8) {
                    ...
                    if (data != null) {
                        ...
                        ReflectionUtil.setValue(packet, true, "k", effect == ParticleEffect.ITEM_CRACK ? packetData
         ...
         *            Center location of the effect
         * @param range
         *            Range in which players will receive the packet (Maximum range for particles is usually 16, but it can differ for some
         *            types)
         * @throws IllegalArgumentException
