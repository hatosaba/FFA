...
public class Scoreboard {
    ...
    //新しいスコアボードを作成する
    public final org.bukkit.scoreboard.Scoreboard board;
    ...
    public Scoreboard(User user){
        this.player = user.asBukkitPlayer();
        this.board = Bukkit.getScoreboardManager().getNewScoreboard();


        this.objective = this.board.registerNewObjective("namelife", "health");
        this.objective.setDisplaySlot(DisplaySlot.BELOW_NAME);
        this.objective.setDisplayName(ChatColor.DARK_RED + "\u2764");
        /*//タイトルはランダムに生成したUUIDの前方16文字を用いる
        objective = board.registerNewObjective(UUID.randomUUID().toString().substring(0, 16), "dummy");
        //表示位置をサイドバーに設定する
    ...
    public void updatelife() {
        for (Player onlinePlayers : Bukkit.getOnlinePlayers()) {
            ...
            this.objective.getScore(player.getName()).setScore((int) (player2).getHealth() + absorpHearts(player2));
        }
    }
    ...
    public Scoreboard(User user, String... defaultTexts){
        this(user);
    ...
    public void setDisplay(boolean display){
        if(display != isDisplay()) player.setScoreboard(display ? board : Bukkit.getScoreboardManager().getNewScoreboard());
    }
